...
def makePassFailHistograms( sample, flag, bindef, var ):
    ...
    hPass = []
    hFail = []
    count_pass = count_fail = 0 #added by me
...
#                if bindef['bins'][ib]['vars'][aVar]['min'] > 89: notflag = '( %s  || !(%s) )' % (flag,flag)
        ...
        count_pass += 1
        count_fail += 1
        print("passI and failI : ", passI, failI)
    ...
    print("countpass and countfail : ", count_pass, count_fail)
    outfile.Close()
...
def getAllEffi( info, bindef ):
    ...
    if not info['mcNominal'] is None and os.path.isfile(info['mcNominal']):
        ...
        print("bin2")
        bin1 = 11
        bin2 = 70   #11 and 70 were used in original egamama
        eP = rt.Double(-1.0)
        eF = rt.Double(-1.0)
